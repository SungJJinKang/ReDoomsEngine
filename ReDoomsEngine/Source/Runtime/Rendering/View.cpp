...
XMFLOAT4X4 FView::Get3DViewProjMatrices(const float InFovInDegrees, const float InScreenWidth, const float InScreenHeight)
{
	...
	XMFLOAT4X4 Result;
	XMStoreFloat4x4(&Result, XMMatrixMultiply(XMMatrixTranspose(XMMatrixLookAtRH(Eye, At, Up)), XMMatrixTranspose(XMMatrixPerspectiveFovRH(FovAngleY, AspectRatio, 0.01f, 125.0f))));
	return Result;
}
...
{
	XMFLOAT4X4 Result;
	XMStoreFloat4x4(&Result, XMMatrixMultiply(XMMatrixTranspose(XMMatrixLookAtRH(Eye, At, Up)), XMMatrixTranspose(XMMatrixOrthographicRH(InWidth, InHeight, 0.01f, 125.0f))));
	return Result;
}
...
void FView::Reset()
{
	Eye = XMVectorSet(0.0f, 15.0f, -30.0f, 0.0f);
	At = XMVectorSet(0.0f, 8.0f, 0.0f, 0.0f);
